class DashboardController < ApplicationController
  def dash
    ...
    # end
    results_with_count = ActiveRecord::Base.connection.execute("select menu_items.name,count(*) FROM menu_items INNER JOIN order_items ON menu_items.id=order_items.menu_item_id WHERE menu_items.menu_id=1  GROUP BY menu_items.id")  ##{pass params from erb for menu_id}
    @dish_and_count = {}
    results_with_count.each { |res| @dish_and_count[res["name"]] = res["count"] }

    results_with_price = ActiveRecord::Base.connection.execute("select menu_items.name,count(*),menu_items.price FROM menu_items INNER JOIN order_items ON menu_items.id=order_items.menu_item_id WHERE menu_items.menu_id=1  GROUP BY menu_items.id")
